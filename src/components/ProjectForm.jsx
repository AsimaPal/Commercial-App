...
function ProjectForm({
  ...
  useEffect(() => {
    if (editingProject) {
      console.log("Project updated:", editingProject); // <-- FIXED: Changed from projectData to editingProject
  ...
  const handleSubmit = async (e) => {
    ...
    const projectData = {
      ...
      deliveryManagerName,
      monthlyData: monthlyData,
      creatorId: userId, // <-- CRUCIAL: Stores the creator's UID
      createdAt: new Date(), // Optional: Add a timestamp
    };

    try {
      // Projects are now stored in a common collection: artifacts/{appId}/projects
      const projectsCollectionRef = collection(
        db,
        `artifacts/${appId}/projects`
      ); // <-- UPDATED PATH

      if (editingProject) {
        // Update existing project in the common collection
        const projectDocRef = doc(projectsCollectionRef, editingProject.id);
        await setDoc(projectDocRef, projectData, { merge: true });
        setMessage("Project updated successfully!");
      } else {
        // Add new project to the common collection
        await addDoc(projectsCollectionRef, projectData);
        setMessage("Project added successfully!");
      }
      setIsError(false);
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-[#F8FDFE] p-5 rounded-xl shadow-sm border border-[#EBF5F7]">
            <h3 className="text-lg font-semibold text-gray-700 mb-4 border-b pb-2 border-gray-200">
              Project Information
            </h3>
            <div className="space-y-4">
              ...
              <div>
                <label
                  ...
                  className="block text-sm font-medium text-gray-700 mb-1"
                >
                  Project Manager Name <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label
                  htmlFor="deliveryManagerName"
                  className="block text-sm font-medium text-gray-700 mb-1"
                >
                  Delivery Manager Name <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
          ...
          <div className="bg-[#F8FDFE] p-5 rounded-xl shadow-sm border border-[#EBF5F7]">
            ...
            {Object.keys(monthlyData).length > 0 && (
              <div className="mt-6">
                <h4 className="text-base font-semibold text-gray-700 mb-3 border-b pb-1.5 border-gray-200">
                  Recorded Monthly Data:
                </h4>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {Object.entries(monthlyData)
                    ...
                    .map(([monthKey, data]) => (
                      <div
                        key={monthKey}
                        className="bg-gray-100 p-2.5 rounded-lg border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 w-full sm:w-auto">
                          <span className="font-semibold text-gray-800 text-sm mb-1 sm:mb-0">
                            {monthKey}:
                          </span>
                          <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-xs sm:grid-cols-3 sm:gap-x-2 sm:gap-y-0">
                            <span className="text-gray-700">
                              <strong className="text-gray-800">Rev:</strong> $
                            ...
                            <span className="text-gray-700">
                              <strong className="text-gray-800">Res:</strong>{" "}
                              {data.members}
                            </span>
                            <span className="text-gray-700">
                              <strong className="text-gray-800">RPE:</strong> $
                              {data.rpe ? data.rpe.toFixed(2) : "N/A"}
                            </span>
                            <span className="text-gray-700">
                              <strong className="text-gray-800">CPE:</strong> $
                              {data.cpe ? data.cpe.toFixed(2) : "N/A"}
                            </span>
                            <span className="text-gray-700">
                              <strong className="text-gray-800">GM%:</strong>{" "}
                              {data.gm ? data.gm.toFixed(2) : "N/A"}%
                            </span>
                          </div>
                        </div>
